...
public class KitEditPotionEffectButton extends KitButton {
    ...
    public ItemStack getButtonItem(Player player) {
        return new ItemBuilder(Material.POTION)
                ...
                .lore(getKit().getEffects().stream().map(effect -> " " + CC.DARK_AQUA + WordUtil.toCapital(effect.getType().getName()) + " " + (effect.getAmplifier() + 1) + CC.GRAY + " - " + CC.WHITE + TimeUtil.millisToTimer(effect.getDuration() / 20 * 1000L)).collect(Collectors.toList()))
