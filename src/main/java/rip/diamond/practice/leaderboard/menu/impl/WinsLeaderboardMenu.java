...
public class WinsLeaderboardMenu extends LeaderboardMenu {
    ...
    public List<Button> getLeaderboardButtons(Player player) {
        ...
        getPlugin().getLeaderboardManager().getWinsLeaderboard()
                .values()
                .stream()
                .sorted(Comparator.comparing(leaderboard -> leaderboard.getKit().getPriority()))
                .forEach(leaderboard -> buttons.add(new Button() {
                    @Override
